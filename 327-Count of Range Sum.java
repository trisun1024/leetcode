class CountOfRangeSum {
    ...
    private int mergeSort(long[] preSum, int start, int end, int lower, int upper) {
        ...
        int count = mergeSort(preSum, start, mid, lower, upper) + mergeSort(preSum, mid + 1, end, lower, upper);

        int left = mid + 1, right = mid + 1, t = mid + 1;
        long[] cache = new long[end - start + 1];
        for (int i = start, r = 0; i <= mid; i++, r++) {
            while (left <= end && preSum[left] - preSum[i] < lower) {
                left++;
            }
            while (right <= end && preSum[right] - preSum[i] <= upper) {
                right++;
            }

...
class NextClosestTime  {
    ...
    /**
     * find the next bigger digit which is no more than upperLimit. If no such digit
     * exists in digits[], return the minimum one i.e. digits[0]
     * 
    ...
    private char findNext(char current, char upperLimit, char[] digits) {
        ...
        while (pos < 4 && (digits[pos] > upperLimit || digits[pos] == current)) { // traverse one by one to find next
                                                                                  // greater digit
            pos++;
        }
